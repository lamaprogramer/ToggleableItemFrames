...
public class ConfigScreen<C extends Config> extends Screen {
    ...
    public void render(MatrixStack context, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < this.widgets.size(); i++) {
            ...
            this.client.textRenderer.drawWithShadow(context, text, posX, posY, 16777215);
        }
    }

    private ButtonWidget saveButton() {
        return new ButtonWidget(
                (((this.width/2) - BUTTON_WIDTH) / 2) + (this.width/2),
                this.height-BUTTON_HEIGHT,
                BUTTON_WIDTH,
                BUTTON_HEIGHT,
                Text.translatable("toggleableitemframes.option.save"), button -> {
                    this.closeScreen();
    ...
    private ButtonWidget cancelButton() {
        return new ButtonWidget(
                ...
                BUTTON_WIDTH,
                BUTTON_HEIGHT,
                Text.translatable("toggleableitemframes.option.cancel"), button -> {
                    this.closeScreen();
                });
    }
